...
import { InventoryBrand, InventoryCategory, InventoryEquipment, InventoryPagination, InventoryProduct, InventoryTag, InventoryVendor , Empleado, Servicio} from 'app/modules/admin/apps/services/list/inventory.types';
...
export class InventoryListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    selectedService: Servicio | null = null;//equipment seleccionado, inicializado como null
    selectedServiceForm: UntypedFormGroup; // Formulario no tipado para el producto seleccionado
  ...
  selectedEmpleado: Empleado | null = null;
    ...
    ngOnInit(): void {
        ...
        // Crear el formulario del servicio seleccionado
        this.selectedServiceForm = this._formBuilder.group({
            servicios_id: [0], // ID del servicio, clave primaria
            nombreResponsableEgreso: [null], // Nombre del responsable del egreso
            cargoSolicitante: [null], // Cargo del solicitante
            informe: [null], // Informe relacionado al servicio
            cargoResponsableEgreso: [null], // Cargo del responsable del egreso
            oficinaSolicitante: [null], // Oficina del solicitante
            fechaRegistro: [null], // Fecha de registro del servicio
            equipo: [null], // Información del equipo asociado
            problema: [null], // Descripción del problema atendido
            telefonoResponsableEgreso: [null], // Teléfono del responsable del egreso
            gestion: [null], // Gestión a la que pertenece
            telefonoSolicitante: [null], // Teléfono del solicitante
            tecnicoAsignado: [null], // ID del técnico asignado
            observaciones: [null], // Observaciones adicionales
            tipoResponsableEgreso: [null], // Tipo de responsable del egreso
            estado: [null], // Estado del servicio (e.g., TERMINADO, PENDIENTE)
            tipoSolicitante: [null], // Tipo del solicitante
            fechaTerminado: [null], // Fecha de finalización del servicio
            oficinaResponsableEgreso: [null], // Oficina del responsable del egreso
            numero: [null], // Número asociado al servicio
            fechaInicio: [null], // Fecha de inicio del servicio
            fechaEgreso: [null], // Fecha de egreso (si aplica)
            ciSolicitante: [null], // CI del solicitante
            nombreSolicitante: [null], // Nombre del solicitante
            tipo: [null], // Tipo de servicio
        ...
        this.bienesData$ = this.bienesService.bienes$; // Asignación del observable reactivo
         ...
         this.services$ = this._inventoryService.services$;
         console.log('services loaded:', this.services$)

        this._inventoryService.services$.pipe(takeUntil(this._unsubscribeAll)).subscribe({
            next: (services) => console.log('services loaded:', services),
            error: (err) => console.error('Error loading services:', err),
        });
        this._inventoryService.services$.pipe(
            takeUntil(this._unsubscribeAll)
        ).subscribe((services) => {
            console.log('services from service:', services);
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    ...
                    return this._inventoryService.getServices(0, 10, 'name', 'asc', query); // Obtener productos
                }),
                map(() => {
        ...
        if (this._sort && this._paginator) {
            ...
            merge(this._sort.sortChange, this._paginator.page).pipe(
                switchMap(() => {
                    ...
                    return this._inventoryService.getServices(this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction); // Obtener productos
                }),
                map(() => {
                    this.isLoading = false; // Indicar que la carga ha finalizado
                }),
    ...
    toggleDetails(servicios_id: number,codigo: string): void {
        // Si el producto ya está seleccionado, cerrar los detalles
        if (this.selectedService && this.selectedService.servicios_id === servicios_id) {
            this.closeDetails();
            return;
        }
    
        // Verifica que `getEquipmentById` devuelva un Observable
        this._inventoryService.getServiceById(servicios_id).subscribe({
            next: (response) => {
                ...
                this.selectedServiceForm.controls['funcionarioasignado'].setValue(response.data.funcionarioasignado); // Copia el valor al input
                this.selectedServiceForm.controls['funcionariousuario'].setValue(response.data.funcionariousuario); // Copia el valor al input
                this.selectedServiceForm.controls['tipo'].setValue(response.data.tipoDescripcion); // Copia el valor al input
                */
                },
            error: (err) => {
    ...
    generateRandomCode(): void {
        ...
        if (this.selectedServiceForm) {
            this.selectedServiceForm.get('codigo')?.setValue(randomCode);
        }
    ...
    getBienes4(): void {
        ...
        const codigoBien = this.selectedServiceForm.get('codigo')?.value;
     ...
     */
    closeDetails(): void {
        this.selectedService = null; // Restablecer el producto seleccionado
    }
    ...
    cycleImages(forward: boolean = true): void {
        const count = this.selectedServiceForm.get('images').value.length; // Número de imágenes
        const currentIndex = this.selectedServiceForm.get('currentImageIndex').value; // Índice de la imagen actual
        ...
        if (forward) {
            this.selectedServiceForm.get('currentImageIndex').setValue(nextIndex); // Actualizar al índice siguiente
        } else {
            this.selectedServiceForm.get('currentImageIndex').setValue(prevIndex); // Actualizar al índice anterior
        }
    }
    ...
    {
        this._inventoryService.createService().subscribe((newEquipment) => {
            this.selectedService = newEquipment; // Establecer el producto como seleccionado
            this.selectedServiceForm.patchValue(newEquipment); // Rellenar el formulario con los datos del nuevo producto
            this._changeDetectorRef.markForCheck(); // Marcar para detección de cambios
    ...
    updateSelectedEquipment2(): void {
        const  equipment = this.selectedServiceForm.getRawValue(); // Obtener los valores del formulario
        delete equipment.currentImageIndex; // Eliminar el índice de imagen actual, ya que no es necesario para el servidor

        this._inventoryService.updateService(this.selectedService.servicios_id, equipment).subscribe(() => {
            this.showFlashMessage('success'); // Mostrar mensaje de éxito
        });
    }
    updateSelectedService(): void {
        const service = this.selectedServiceForm.getRawValue(); // Obtener los valores del formulario
    
        // Reemplaza el campo `tipo` con `tiposId` al enviar al servidor
        if (this.selectedServiceForm.controls['tiposId']?.value) {
            service.tipo = this.selectedServiceForm.controls['tiposId'].value; // Usa el ID en lugar del texto
        }
    
        delete service.currentImageIndex; // Eliminar el índice de imagen actual, ya que no es necesario para el servidor
    
        console.log('Datos a actualizar:', service); // Debug para verificar los datos
    
        this._inventoryService.updateService(this.selectedService.servicios_id, service).subscribe(() => {
            this.showFlashMessage('success'); // Mostrar mensaje de éxito
        }, (error) => {
    ...
    deleteSelectedEquipment(): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
                ...
                this._inventoryService.deleteService(equipment.equipos_id).subscribe(() => { // Eliminar el producto en el servidor
                    this.closeDetails(); // Cerrar los detalles del producto
                });
            }
        });
     ...
     */
     deleteSelectedService(): void {
        const confirmation = this._fuseConfirmationService.open({ // Abrir diálogo de confirmación
            title  : 'Delete service', // Título del diálogo
            message: 'Are you sure you want to remove this product? This action cannot be undone!', // Mensaje del diálogo
            actions: {
                confirm: {
                    label: 'Delete', // Etiqueta del botón de confirmación
        ...
        confirmation.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
                const service = this.selectedServiceForm.getRawValue(); // Obtener los valores del formulario
                this._inventoryService.deleteService(service.servicios_id).subscribe(() => { // Eliminar el producto en el servidor
                    this.closeDetails(); // Cerrar los detalles del producto
                });
            }
        });
